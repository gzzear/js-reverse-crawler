...
class PddParserV2:
    def __init__(self, root):
        self.root = root
        self.root.title("PDD 数据解析专家 v2.0 (本地路径版)")
    ...
    def build_ui(self):
        ...
        ttk.Label(nav, text=" PDD DATA PARSER V2 ", style="Header.TLabel").pack(side=tk.LEFT, padx=30)
        ttk.Label(nav, text=" 本地路径版 | 图片下载到本地 ", font=("Microsoft YaHei", 9), background=COLOR_NAVY_DARK,
        ...
        ttk.Label(f2, text="图片保存目录", font=("Microsoft YaHei", 10, "bold")).pack(side=tk.LEFT)
        self.img_dir = tk.StringVar()
        ttk.Entry(f2, textvariable=self.img_dir).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=20)
        ttk.Button(f2, text="选择目录", width=10, command=self.select_img_dir).pack(side=tk.LEFT)
        ...
        self.run_btn = ttk.Button(ctrl_fm, text="🚀 开始解析（下载图片）", style="Primary.TButton",
    ...
    def start_task(self):
        if not self.in_path.get() or not self.out_path.get() or not self.img_dir.get():
            messagebox.showwarning("提示", "请先配置输入源、图片保存目录和Excel保存位置")
            return
    ...
    def work_thread(self):
        ...
        if data_rows:
            ...
            self.progress_text.set(f"解析完成，开始下载图片并生成Excel...")
    ...
    def generate_excel(self, results):
        ...
        # 收集所有图片下载任务（按商品分组）
        download_tasks = []  # [(url, goods_id, goods_name, img_type, img_index)]
        row_image_info = {}  # 映射: row_idx -> {'goods_id': '', 'goods_name': '', 'sku': [urls], 'main': [urls]}
        ...
        for r_idx, (row_data, sku_imgs, main_imgs) in enumerate(results, start=2):
            ...
            goods_id = row_data[0]  # 产品ID
            goods_name = row_data[1][:30]  # 标题（截取前30个字符，避免文件夹名过长）
            
            # 记录每行的图片信息
            row_image_info[r_idx] = {
                'goods_id': goods_id,
            ...
            # 添加规格图片下载任务
            for idx, url in enumerate(row_image_info[r_idx]['sku'], start=1):
                download_tasks.append((url, goods_id, goods_name, '规格图', idx))
            
            # 添加主图下载任务
            for idx, url in enumerate(row_image_info[r_idx]['main'], start=1):
                download_tasks.append((url, goods_id, goods_name, '主图', idx))
        ...
        if total_images == 0:
            ...
            self.logger.log(f"开始下载 {total_images} 张图片到指定目录...", "INFO")
        
        img_cache = self.bulk_download_with_progress(download_tasks, img_base_dir) if total_images > 0 else {}
        ...
        # 将本地绝对路径写入Excel单元格
        self.img_progress_text.set("正在填充本地图片路径...")
        
        for r_idx, info in row_image_info.items():
            goods_id = info['goods_id']
            goods_name = info['goods_name']
            
            # 规格图片绝对路径（J列，第10列）
            sku_paths = []
            for idx, url in enumerate(info['sku'], start=1):
                key = (url, goods_id, goods_name, '规格图', idx)
                if key in img_cache and img_cache[key]:
                    sku_paths.append(img_cache[key])
            if sku_paths:
                ws.cell(row=r_idx, column=10, value='||'.join(sku_paths))
            
            # 主图绝对路径（K列，第11列）
            main_paths = []
            for idx, url in enumerate(info['main'], start=1):
                key = (url, goods_id, goods_name, '主图', idx)
                if key in img_cache and img_cache[key]:
                    main_paths.append(img_cache[key])
            if main_paths:
                ws.cell(row=r_idx, column=11, value='||'.join(main_paths))
        ...
        ws.column_dimensions['J'].width = 60  # 规格图片（本地路径）
        ws.column_dimensions['K'].width = 60  # 图片（本地路径）
        ...
        try:
            ...
            self.img_progress_text.set(f"✓ 已下载 {downloaded} 张图片")
            
            # 生成统计报告
            report = f"数据已成功导出至 Excel\n\n"
            report += f"商品数量: {len(results)}\n"
            report += f"导出格式: 商品上架格式（绝对路径版）\n"
            report += f"图片下载: {downloaded}/{total_images}\n"
            report += f"图片格式: 绝对路径（用||分隔）\n"
    ...
    def bulk_download_with_progress(self, tasks, base_dir):
        """批量下载图片（带实时进度反馈 + 失败重试机制）
        
        Args:
            tasks: 下载任务列表 [(url, goods_id, goods_name, img_type, img_index), ...]
            base_dir: 图片保存根目录
        
        Returns:
            dict: {(url, goods_id, goods_name, img_type, img_index): 绝对路径}
        """
        total = len(tasks)
        ...
        with ThreadPoolExecutor(max_workers=50) as exe:
            future_to_task = {
                exe.submit(self.download_image_to_category, task, base_dir): task 
                for task in tasks
            }
            ...
            for future in as_completed(future_to_task):
                ...
                self.img_progress_text.set(
                    ...
                    f"成功: {len(results)} | 失败: {len(failed_tasks)} | "
        ...
        while failed_tasks and retry_count < max_retries:
            retry_count += 1
            self.logger.log(f"第 {retry_count} 次重试，共 {len(failed_tasks)} 张失败图片...", "WARNING")
            ...
            with ThreadPoolExecutor(max_workers=20) as exe:
                future_to_task = {
                    exe.submit(self.download_image_to_category, task, base_dir, retry_count): task 
                ...
                for idx, future in enumerate(as_completed(future_to_task)):
                    ...
                    # 更新进度
                    self.img_progress_text.set(
                        f"第{retry_count}次重试: {idx + 1}/{len(retry_tasks)} | "
                        f"本轮成功: {retry_success} | 仍失败: {len(failed_tasks)}"
                    )
                    self.root.update_idletasks()
    ...
    def download_image_to_category(self, task, base_dir, retry_num=0):
        """下载图片到分类目录（中文命名）
        
        Args:
            task: (url, goods_id, goods_name, img_type, img_index)
            base_dir: 基础目录
            retry_num: 重试次数（0表示首次下载）
        
        Returns:
            str: 图片的绝对路径，失败返回None
        """
        try:
            url, goods_id, goods_name, img_type, img_index = task
            
            # 清理商品名称，移除文件系统不支持的字符
            safe_name = re.sub(r'[<>:"/\\|?*]', '_', goods_name)
            
            # 构建目录：基础目录/商品ID_商品名称/
            goods_folder = os.path.join(base_dir, f"{goods_id}_{safe_name}")
            os.makedirs(goods_folder, exist_ok=True)
            
            # 构建文件名：图片类型_序号.jpg
            filename = f"{img_type}_{img_index}.jpg"
            file_path = os.path.join(goods_folder, filename)
            ...
            # 下载图片
            timeout = (3, 5) if retry_num == 0 else (5 + retry_num * 2, 10 + retry_num * 3)
            ...
            if r.status_code == 200:
                ...
                with PILImage.open(img_data) as img:
                    img = img.convert('RGB')
                    img.thumbnail((800, 800), PILImage.Resampling.LANCZOS)  # 保持较高质量
